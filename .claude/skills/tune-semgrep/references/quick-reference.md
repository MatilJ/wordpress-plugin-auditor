...
rules:
  - id: rule-id              # Lowercase with hyphens
    languages: [php]         # Target language(s)
...
> **⚠️ `metavariable-regex` is prefix-anchored, not substring search.** It behaves
> like Python's `re.match()` — anchored at position 0 of the captured string, but
> NOT required to consume the whole string. `regex: "(?i)usercan"` matches
> `"userCanExpirePosts"` (starts with it) but does **NOT** match
> `"myplugin_user_can_manage"` (doesn't start with it). If you want "contains"
> semantics, you must write the leading wildcard yourself:
> `regex: "(?i).*(usercan|can_manage).*"`. Confirmed empirically (recent Semgrep)
> — trailing `.*` is not required (match doesn't need to consume the string) but
> costs nothing and documents intent. Verify any new regex with `semgrep --test`
> against a case where the target string does NOT start with your keyword —
> testing only the trivial "keyword at position 0" case will not catch this.

> **⚠️ A metavariable cannot be a partial prefix inside a quoted string literal
> pattern.** `pattern: add_filter('rest_prepare_$TYPE', ...)` does **not** mean
> "any string starting with rest_prepare_" — it silently matches nothing (no
> error, no warning). Capture the whole argument with a bare metavariable and
> constrain it with `metavariable-regex` instead:
> ```yaml
> patterns:
...
> ```
> This is easy to miss because the rule still validates and still runs without
> error — it just never matches. If a pattern that "should obviously match"
> produces zero findings, suspect this before suspecting anything else (join
> mode, login, environment).
